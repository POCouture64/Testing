...
# -----------------------------------------------------------------------------
# E. Variable agreement
# -----------------------------------------------------------------------------

top_rf_vars <- rf_results$pooled_importance$Variable[1:min(10, nrow(rf_results$pooled_importance))]
...
cat("Agreement (top RF & significant LR):\n")
...
# Forest plot (only if significant predictors exist)
if(n_sig_10 > 0) {
  ...
  p_forest <- ggplot(forest_data, aes(x = OR, y = Variable)) +
    ...
    labs(title = "Odds Ratios for BZRA Discontinuation",
         subtitle = paste0("Pooled across ", mids_with_subscales$m, " imputations"),
...
# -----------------------------------------------------------------------------
# H. Extended Summary and Interpretation
# -----------------------------------------------------------------------------
...
cat("  • Mean AUC:", round(lr_auc, 3), "±", round(lr_performance_summary$SD[1], 3), "\n")
cat("  • RF vs LR AUC difference:", round(auc_diff, 3), "\n")
...
if(n_sig_05 > 0) {
  ...
  cat("\n  ⚠ No predictors reached significance at p < .05\n")
}

if(n_sig_10 > n_sig_05) {
  cat("\n\nMarginally significant predictors (p < .10):\n")
  marg_vars <- or_results %>% filter(p.value >= 0.05 & p.value < 0.10) %>% arrange(p.value)
  for(i in 1:nrow(marg_vars)) {
    cat("  • ", marg_vars$term[i], 
        " (OR=", round(marg_vars$OR[i], 2), 
        ", p=", format.pval(marg_vars$p.value[i], digits = 3), ")\n", sep = "")
  }
}

cat("\n\nSTRONGEST EFFECTS (by OR magnitude):\n")
top_or <- or_results %>%
  filter(!is.na(OR)) %>%
...
if(nrow(top_or) > 0) {
  for(i in 1:nrow(top_or)) {
    direction <- ifelse(top_or$OR[i] > 1, "↑ increases", "↓ decreases")
    cat("  ", i, ". ", as.character(top_or$term[i]), 
        " (OR = ", round(top_or$OR[i], 2), " ", direction, " discontinuation)\n", sep = "")
  }
}
...
cat("  • AUC = ", round(lr_auc, 3), " indicates ", 
    ifelse(lr_auc >= 0.80, "EXCELLENT", 
           ifelse(lr_auc >= 0.70, "GOOD", 
                  ifelse(lr_auc >= 0.60, "FAIR", "POOR"))), 
    " discrimination\n", sep = "")

if(auc_diff < 0.05) {
  cat("  • RF and LR perform similarly (diff = ", round(auc_diff, 3), 
      ") → Strong validation ✓\n", sep = "")
} else if(auc_diff < 0.10) {
  cat("  • RF slightly outperforms LR (diff = ", round(auc_diff, 3), 
      ") → May have non-linear effects\n", sep = "")
} else {
  cat("  • RF substantially outperforms LR (diff = ", round(auc_diff, 3), 
      ") → Complex interactions likely\n", sep = "")
}

if(length(agreement) > 0) {
  cat("  • ", length(agreement), " variable(s) significant in BOTH methods → Most robust predictors\n", sep = "")
}

cat("\n\nVARIABLE AGREEMENT ANALYSIS:\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("Top 10 RF variables:\n")
for(i in seq_along(top_rf_vars)) {
  in_lr <- top_rf_vars[i] %in% sig_lr_vars_05
  marker <- ifelse(in_lr, "✓ [AGREE]", "  ")
  cat("  ", i, ". ", top_rf_vars[i], " ", marker, "\n", sep = "")
}

cat("\nVariables significant in BOTH methods (most reliable):\n")
if(length(agreement) > 0) {
  for(v in agreement) {
    or_val <- or_results$OR[or_results$term == v]
    p_val <- or_results$p.value[or_results$term == v]
    cat("  ✓ ", v, " (OR=", round(or_val, 2), ", p=", 
        format.pval(p_val, digits = 3), ")\n", sep = "")
  }
} else {
  cat("  (None - interpret results cautiously)\n")
}

cat("\n\nFOR YOUR MANUSCRIPT:\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat('  "Logistic regression was conducted using the same VSURF-selected\n')
cat('   variables to validate Random Forest findings. Models were fitted\n')
cat('   across ', mids_with_subscales$m, ' multiply-imputed datasets and results were\n', sep = "")
cat('   pooled using Rubin\'s rules. The logistic regression model\n')
cat('   achieved an AUC of ', round(lr_auc, 3), ' (95% CI: ',
    lr_performance_summary$CI_95[1], '),\n', sep = "")

if(auc_diff < 0.05) {
  cat('   closely matching the Random Forest model (AUC = ', round(rf_auc, 3), ',\n', sep = "")
  cat('   difference = ', round(auc_diff, 3), '), confirming the robustness of our\n', sep = "")
  cat('   findings.')
} else if(rf_auc > lr_auc) {
  cat('   compared to the Random Forest model (AUC = ', round(rf_auc, 3), ').\n', sep = "")
  cat('   The superior Random Forest performance suggests non-linear\n')
  cat('   relationships or interactions among predictors.')
} else {
  cat('   exceeding the Random Forest model (AUC = ', round(rf_auc, 3), ').\n', sep = "")
  cat('   This suggests linear relationships adequately capture the\n')
  cat('   predictive structure.')
}
...
if(n_sig_05 > 0) {
  ...
  for(i in 1:nrow(sig_top)) {
    cat('   • ', as.character(sig_top$term[i]), ' (OR = ', round(sig_top$OR[i], 2), ', 95% CI [', 
        round(sig_top$OR_lower[i], 2), ', ', round(sig_top$OR_upper[i], 2), 
        '], p ', format.pval(sig_top$p.value[i], digits = 2), ')\n', sep = "")
  }
}
cat('"\n\n')

cat("ODDS RATIO INTERPRETATION GUIDE:\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("  • OR > 1: Higher values → INCREASED odds of discontinuation\n")
cat("  • OR < 1: Higher values → DECREASED odds of discontinuation\n")
cat("  • OR = 1: No association\n\n")
cat("  Examples:\n")
cat("    - OR = 1.5  → 50% increase in odds (1.5x)\n")
cat("    - OR = 2.0  → 100% increase (2x odds)\n")
cat("    - OR = 0.5  → 50% decrease in odds (half)\n")
cat("    - OR = 0.72 → 28% decrease in odds\n\n")
cat("  Confidence Intervals:\n")
cat("    - If CI includes 1.0 → effect is NOT significant\n")
cat("    - Narrower CI → more precise estimate\n\n")

cat("VALIDATION STATUS:\n")
cat("═══════════════════════════════════════════════════════════════\n")
if(auc_diff < 0.05 && length(agreement) >= 1) {
...
} else if(auc_diff < 0.10 && length(agreement) >= 1) {
  cat("  ✓✓ GOOD VALIDATION\n")
  cat("      • RF and LR show similar performance\n")
  cat("      • At least one variable agrees across methods\n")
  cat("      • Results are reasonably consistent\n")
} else if(length(agreement) >= 1) {
  ...
  cat("      • May need larger sample or different predictors\n")
}

if(n_sig_05 == 0) {
  cat("\n⚠ NOTE: No significant predictors at p < .05\n")
  cat("  Possible reasons:\n")
  cat("  • Small sample size → limited power\n")
  cat("  • Weak effects → hard to detect\n")
  cat("  • High collinearity → shared variance\n")
  cat("  • Multiple imputation → conservative pooling\n")
}

if(lr_auc < 0.60) {
  cat("\n⚠ WARNING: Poor discrimination (AUC < 0.60)\n")
  cat("  • Outcome may not be predictable with current variables\n")
  cat("  • Consider additional predictors or alternative approaches\n")
}

cat("\n\nNEXT STEPS:\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("  1. Review LR_odds_ratios_forest_plot.png\n")
cat("     → Visual summary of effect sizes\n\n")
cat("  2. Check LR_ROC_curve.png\n")
cat("     → Compare discrimination visually\n\n")
cat("  3. Examine RF_vs_LR_comparison.png\n")
cat("     → Assess model agreement\n\n")
cat("  4. Focus on variables with cross-method agreement\n")
cat("     → These are your most reliable findings\n\n")
cat("  5. Consider clinical interpretation\n")
cat("     → What do the significant predictors tell you?\n\n")

cat("FILES CREATED:\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("  • LR_validation_results.rds       (Full results object)\n")
cat("  • LR_odds_ratios.csv              (Table of all ORs)\n")
cat("  • LR_performance_summary.csv      (Performance metrics)\n")
cat("  • RF_vs_LR_comparison.png         (AUC comparison)\n")
if(n_sig_10 > 0) {
  cat("  • LR_odds_ratios_forest_plot.png  (Effect size visualization)\n")
}
cat("  • LR_ROC_curve.png                (Discrimination curve)\n")

cat("\n✓ Logistic Regression validation complete!\n")
