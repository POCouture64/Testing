...
critical_vars_to_add <- c(
  ...
  "med_burden_1", "medburden_2", "medburden_3", "med_burden_4",
...
# Medication burden
med_burden_vars <- c("med_burden_1", "medburden_2", "medburden_3", "med_burden_4")
...
cat("\n")
valid_results <- convergence_df[!convergence_dffailed & !is.na(convergence_df
max_rhat), ]
...
if(nrow(converged_rows) > 0) {
    ...
    cat("  Max R-hat:", round(converged_rowsmaxrhat[convergedrowsmax_rhat[converged_rows
maxr​hat[convergedr​owsmaxit == optimal_maxit], 4), "\n")
  } else {
    optimal_maxit <- valid_resultsmaxit[which.min(validresultsmaxit[which.min(valid_results
maxit[which.min(validr​esultsmax_rhat)]
    cat("⚠ No maxit achieved R-hat < 1.05, using best available: maxit =", optimal_maxit, "\n")
    cat("  Max R-hat:", round(valid_resultsmaxrhat[validresultsmax_rhat[valid_results
maxr​hat[validr​esultsmaxit == optimal_maxit], 4), "\n")
  }
} else {
...
if(!is.null(best_result) && !best_result$failed) {
  rhat_vals <- best_resultrhatvalues[!is.na(bestresultrhat_values[!is.na(best_result
rhatv​alues[!is.na(bestr​esultrhat_values)]
