...
validate_composite <- function(data, items, composite_name) {
  ...
  cat(ifelse(decision == "combine", " ✓ COMBINE POST-IMPUTATION\n", " ✗ KEEP SEPARATE\n"))
...
cat("\n✓ Saved: missingness_by_variable.csv\n")
...
if(length(missing_critical) > 0) {
  cat("⚠️  CRITICAL: Missing variables detected before mice initialization:\n")
...
if(!all(vars_in_pred %in% vars_in_data) || !all(vars_in_method %in% vars_in_data)) {
  cat("⚠️  Fixing predictor matrix/method mismatches...\n")
  ...
  cat("     ✓ Predictor matrix dimensions:", nrow(pred), "x", ncol(pred), "\n")
  cat("     ✓ Method vector length:", length(method), "\n\n")
}
```
...
```{r}
#==============================================================================
# LPA WITH MULTIPLE IMPUTATION - USING ALL 35 IMPUTED DATASETS
# Approach: Fit LPA on each imputation, then combine results
#==============================================================================
...
cat("\n")
cat("═══════════════════════════════════════════════════════════════\n")
cat("PART 1: LATENT PROFILE ANALYSIS - WITH MULTIPLE IMPUTATION\n")
cat("═══════════════════════════════════════════════════════════════\n\n")
...
cat("STAGE 1 OBJECTIVE: Identify distinct personality-based patient profiles\n")
cat("Using ALL", mids_obj$m, "imputations for robust estimates\n")
cat("This is DESCRIPTIVE, not predictive\n\n")

# -----------------------------------------------------------------------------
# Define personality variables for profiling
# -----------------------------------------------------------------------------
...
# Get first imputation to check variables
...
# -----------------------------------------------------------------------------
# STEP 1: Fit LPA on each imputed dataset
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("STEP 1: Fitting LPA on each of", mids_obj$m, "imputed datasets\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# Store results from each imputation
lpa_results_list <- list()
bic_results <- list()
selected_k_list <- numeric(mids_obj$m)

set.seed(123)

for(m in 1:mids_obj$m) {
  cat("Processing imputation", m, "of", mids_obj$m, "...\n")
  
  # Extract imputation
  imp_data <- complete(mids_obj, m)
  
  # Prepare LPA data (no missing values needed for LPA)
  lpa_data_m <- imp_data %>%
    select(all_of(personality_vars))
  
  # Scale variables
  lpa_scaled_m <- scale(lpa_data_m)
  
  # Fit LPA models (1-8 profiles)
  lpa_fit_m <- Mclust(lpa_scaled_m, G = 1:8, verbose = FALSE)
  
  # Store results
  lpa_results_list[[m]] <- lpa_fit_m
  bic_results[[m]] <- lpa_fit_m$BIC
  selected_k_list[m] <- lpa_fit_m$G
  
  cat("  → Best k =", lpa_fit_m$G, ", BIC =", round(lpa_fit_m$bic, 2), "\n")
}

cat("\n")

# -----------------------------------------------------------------------------
# STEP 2: Determine consensus number of profiles
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("STEP 2: Determining consensus number of profiles\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# Most common k selected
k_table <- table(selected_k_list)
cat("Number of profiles selected across imputations:\n")
print(k_table)
cat("\n")

# Use modal (most common) k
consensus_k <- as.numeric(names(k_table)[which.max(k_table)])
cat("Consensus number of profiles:", consensus_k, "\n")
cat("Selected in", max(k_table), "out of", mids_obj$m, "imputations\n\n")

# If there's a tie or concern, you could also use median/mean
cat("Distribution of selected k:\n")
cat("  Mode:", consensus_k, "\n")
cat("  Median:", median(selected_k_list), "\n")
cat("  Mean:", round(mean(selected_k_list), 2), "\n\n")

# -----------------------------------------------------------------------------
# STEP 3: Refit LPA with consensus k on all imputations
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("STEP 3: Refitting LPA with k =", consensus_k, "on all imputations\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

lpa_consensus_list <- list()
profile_assignments_list <- list()
entropy_list <- numeric(mids_obj$m)
...
for(m in 1:mids_obj$m) {
  ...
  lpa_fit_m <- Mclust(lpa_scaled_m, G = consensus_k, verbose = FALSE)
  
  # Calculate entropy
  entropy_m <- 1 - (sum(-lpa_fit_m$z * log(lpa_fit_m$z), na.rm = TRUE) / 
                    (nrow(lpa_scaled_m) * log(consensus_k)))
  
  # Store
  lpa_consensus_list[[m]] <- lpa_fit_m
  profile_assignments_list[[m]] <- lpa_fit_m$classification
  entropy_list[m] <- entropy_m
}

cat("Entropy across imputations:\n")
cat("  Mean:", round(mean(entropy_list), 3), "\n")
cat("  SD:", round(sd(entropy_list), 3), "\n")
cat("  Range:", round(min(entropy_list), 3), "-", round(max(entropy_list), 3), "\n\n")

# -----------------------------------------------------------------------------
# STEP 4: Obtain consensus profile assignments
# Method: Use first imputation as reference, assign based on modal profile
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("STEP 4: Creating consensus profile assignments\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# Use first imputation as reference
reference_lpa <- lpa_consensus_list[[1]]
reference_profiles <- profile_assignments_list[[1]]

# For simplicity, use the modal profile assignment across imputations
# This is more stable than using just one imputation
n_obs <- length(reference_profiles)

# Create matrix of profile assignments (rows = obs, cols = imputations)
profile_matrix <- do.call(cbind, profile_assignments_list)

# Get modal profile for each observation
modal_profiles <- apply(profile_matrix, 1, function(x) {
  as.numeric(names(sort(table(x), decreasing = TRUE)[1]))
})

# Create final dataset with consensus profiles
consensus_data <- complete(mids_obj, 1) %>%
  select(all_of(personality_vars)) %>%
  mutate(
    profile = factor(modal_profiles,
                    levels = 1:consensus_k,
                    labels = paste0("Profile_", 1:consensus_k))
  )

cat("Consensus profile distribution:\n")
print(table(consensus_data$profile))
cat("\nProportions (%):\n")
print(round(100 * prop.table(table(consensus_data$profile)), 1))
cat("\n\n")

# Calculate assignment stability (what % of imputations agree with modal?)
assignment_stability <- apply(profile_matrix, 1, function(x) {
  modal_val <- as.numeric(names(sort(table(x), decreasing = TRUE)[1]))
  mean(x == modal_val)
})

cat("Profile assignment stability:\n")
cat("  Mean agreement:", round(100 * mean(assignment_stability), 1), "%\n")
cat("  Median agreement:", round(100 * median(assignment_stability), 1), "%\n")
cat("  Min agreement:", round(100 * min(assignment_stability), 1), "%\n\n")

# -----------------------------------------------------------------------------
# STEP 5: Pool profile characteristics across imputations
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("STEP 5: Pooling profile characteristics across imputations\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# For each imputation, calculate profile means
profile_means_list <- list()

for(m in 1:mids_obj$m) {
  imp_data <- complete(mids_obj, m) %>%
    select(all_of(personality_vars)) %>%
    mutate(profile = factor(profile_assignments_list[[m]],
                           levels = 1:consensus_k,
                           labels = paste0("Profile_", 1:consensus_k)))
  
  profile_means_m <- imp_data %>%
    group_by(profile) %>%
    summarise(across(all_of(personality_vars), mean, na.rm = TRUE),
              .groups = "drop") %>%
    mutate(imputation = m)
  
  profile_means_list[[m]] <- profile_means_m
}

# Combine and average across imputations
pooled_profile_means <- bind_rows(profile_means_list) %>%
  group_by(profile) %>%
  summarise(across(all_of(personality_vars), 
                   list(mean = mean, sd = sd)),
            .groups = "drop")

cat("Pooled profile characteristics (averaged across", mids_obj$m, "imputations):\n\n")

# Create a cleaner table
profile_summary <- pooled_profile_means %>%
  select(profile, ends_with("_mean")) %>%
...
# -----------------------------------------------------------------------------
# STEP 6: Statistical comparison of profiles (pooled)
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("PERSONALITY CHARACTERISTICS BY PROFILE (POOLED)\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# Use consensus profiles with first imputation for TableOne
profile_table <- CreateTableOne(
  vars = personality_vars,
  strata = "profile",
  data = consensus_data,
  test = TRUE
)
...
cat("\n")
cat("Note: Statistics based on consensus profile assignments\n")
cat("      Means are pooled across", mids_obj$m, "imputations\n\n")

# -----------------------------------------------------------------------------
# STEP 7: Visualizations using reference imputation
# -----------------------------------------------------------------------------

cat("Creating profile visualizations...\n\n")

# Use reference LPA for BIC plot
bic_plot <- fviz_mclust(reference_lpa, "BIC", palette = "jco") +
  labs(title = "LPA Model Selection: BIC Values (Reference Imputation)",
       subtitle = paste0("Consensus k = ", consensus_k, " across ", mids_obj$m, " imputations")) +
  theme_minimal()

ggsave("lpa_bic_selection_MI.png", plot = bic_plot, width = 10, height = 6, dpi = 300)

# Classification plot using consensus profiles
consensus_scaled <- scale(consensus_data %>% select(-profile))
pca_result <- prcomp(consensus_scaled, scale. = FALSE)
pca_data <- data.frame(
  PC1 = pca_result$x[, 1],
  PC2 = pca_result$x[, 2],
  profile = consensus_data$profile
)

class_plot <- ggplot(pca_data, aes(x = PC1, y = PC2, color = profile)) +
  geom_point(alpha = 0.6, size = 2) +
  scale_color_brewer(palette = "Set1") +
  theme_minimal() +
  labs(title = paste("Patient Profiles (k =", consensus_k, ") - Consensus Assignment"),
       subtitle = paste0("Based on ", mids_obj$m, " imputed datasets"),
       x = "First Principal Component",
       y = "Second Principal Component",
       color = "Profile")

ggsave("lpa_profiles_descriptive_MI.png", plot = class_plot, 
       width = 10, height = 8, dpi = 300)

# Heatmap with pooled means
...
p_heatmap <- ggplot(heatmap_data, aes(x = variable, y = profile, fill = z_score)) +
  ...
  labs(title = paste("Personality Profiles (k =", consensus_k, ") - Pooled Estimates"),
       subtitle = paste0("Averaged across ", mids_obj$m, " imputations - Red = Higher, Blue = Lower"),
       x = NULL, y = "Profile")

ggsave("lpa_profile_heatmap_MI.png", plot = p_heatmap, 
       width = 12, height = 6, dpi = 300)

cat("✓ Profile visualizations saved\n\n")

# -----------------------------------------------------------------------------
# STEP 8: Exploratory discontinuation comparison (pooled across imputations)
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("EXPLORATORY: Profile differences in discontinuation\n")
cat("(Descriptive comparison - NOT predictive modeling)\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# Calculate discontinuation rates pooled across all imputations
discont_rates_list <- list()

for(m in 1:mids_obj$m) {
  imp_data <- complete(mids_obj, m) %>%
    mutate(row_id = row_number()) %>%
    select(row_id, scrn_stopped_bzra) %>%
    filter(!is.na(scrn_stopped_bzra))
  
  # Merge with consensus profiles
  imp_with_profile <- consensus_profiles_df %>%
    left_join(imp_data, by = "row_id") %>%
    filter(!is.na(scrn_stopped_bzra)) %>%
    mutate(discontinued = as.numeric(as.character(scrn_stopped_bzra)))
  
  # Calculate rates by profile
  rates_m <- imp_with_profile %>%
    group_by(profile) %>%
    summarise(
      N = n(),
      N_discontinued = sum(discontinued, na.rm = TRUE),
      Rate = mean(discontinued, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    mutate(imputation = m)
  
  discont_rates_list[[m]] <- rates_m
}

# Pool discontinuation rates
pooled_discont <- bind_rows(discont_rates_list) %>%
  group_by(profile) %>%
  summarise(
    N = mean(N),  # Should be same across imputations
    Mean_Rate = mean(Rate),
    SD_Rate = sd(Rate),
    SE_Rate = SD_Rate / sqrt(mids_obj$m),
    CI_lower = Mean_Rate - 1.96 * SE_Rate,
    CI_upper = Mean_Rate + 1.96 * SE_Rate,
    .groups = "drop"
  ) %>%
  mutate(
    Rate_Pct = round(100 * Mean_Rate, 1),
    CI_lower_Pct = round(100 * CI_lower, 1),
    CI_upper_Pct = round(100 * CI_upper, 1)
  )

cat("Discontinuation rates by profile (POOLED ACROSS", mids_obj$m, "IMPUTATIONS):\n")
print(pooled_discont %>% 
        select(profile, N, Rate_Pct, CI_lower_Pct, CI_upper_Pct))
cat("\n")

# Chi-square test using first imputation (for reference)
first_imp_test <- complete(mids_obj, 1) %>%
  mutate(row_id = row_number()) %>%
  inner_join(consensus_profiles_df, by = "row_id") %>%
  filter(!is.na(scrn_stopped_bzra))

if(nrow(first_imp_test) > 0) {
  chisq_test <- chisq.test(table(first_imp_test$profile, 
                                 first_imp_test$scrn_stopped_bzra))
  cat("Chi-square test (exploratory, first imputation):\n")
...
# Save plot with pooled rates
p_explore <- ggplot(pooled_discont, 
  ...
  scale_fill_brewer(palette = "Set2") +
  theme_minimal() +
  labs(title = "EXPLORATORY: Discontinuation by Profile (Pooled)",
       subtitle = paste0("Rates averaged across ", mids_obj$m, " imputations"),
       y = "Discontinuation Rate (%)", x = "Profile") +
  theme(legend.position = "none")

ggsave("lpa_exploratory_discontinuation_MI.png", plot = p_explore, 
       width = 10, height = 6, dpi = 300)
...
lpa_mi_results <- list(
  ...
  # Profile assignments
  consensus_profiles = consensus_data$profile,
  assignment_stability = assignment_stability,
  
  # Model information
  mean_entropy = mean(entropy_list),
  entropy_sd = sd(entropy_list),
  
  # Pooled characteristics
  pooled_means = profile_summary,
  
  # All LPA fits
  lpa_fits = lpa_consensus_list,
  
  # Distribution
  profile_distribution = table(consensus_data$profile)
)

saveRDS(lpa_mi_results, "lpa_results_multiple_imputation.rds")
cat("✓ Saved: lpa_results_multiple_imputation.rds\n\n")

# Also save consensus profiles to merge with other data later
consensus_profiles_df <- data.frame(
  row_id = 1:nrow(consensus_data),
  profile = consensus_data$profile,
  stability = assignment_stability
)

saveRDS(consensus_profiles_df, "consensus_profiles.rds")
cat("✓ Saved: consensus_profiles.rds\n\n")
...
cat("═══════════════════════════════════════════════════════════════\n")
cat("LPA WITH MULTIPLE IMPUTATION - COMPLETE\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

cat("RESULTS SUMMARY:\n")
cat("  Imputations analyzed:", mids_obj$m, "\n")
cat("  Consensus profiles:", consensus_k, "\n")
cat("  Mean entropy:", round(mean(entropy_list), 3), "±", round(sd(entropy_list), 3), "\n")
cat("  Mean assignment stability:", round(100 * mean(assignment_stability), 1), "%\n\n")
...
cat("  ✓ Uses all", mids_obj$m, "imputations (not just one)\n")
cat("  ✓ Accounts for imputation uncertainty\n")
cat("  ✓ More stable profile assignments\n")
cat("  ✓ Pooled estimates reduce bias\n")
cat("  ✓ Provides assignment stability metric\n\n")

cat("OUTPUT FILES:\n")
cat("  ✓ lpa_bic_selection_MI.png\n")
cat("  ✓ lpa_profiles_descriptive_MI.png\n")
cat("  ✓ lpa_profile_heatmap_MI.png\n")
if(exists("p_explore")) {
  cat("  ✓ lpa_exploratory_discontinuation_MI.png\n")
}
cat("  ✓ lpa_results_multiple_imputation.rds\n")
cat("  ✓ consensus_profiles.rds\n\n")

cat("NEXT: Predictive regression modeling (below)\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

# =============================================================================
# PART 2: PREDICTIVE MODELING WITH MULTIPLE IMPUTATION
# =============================================================================
...
cat("OBJECTIVE: Predict BZRA discontinuation using:\n")
cat("  • Demographic variables\n")
cat("  • Personality variables\n")
cat("  • Properly pooled across all", mids_obj$m, "imputations\n\n")
...
# Personality (same as LPA)
personality_predictors <- personality_vars

# Check which exist in data
first_imp_check <- complete(mids_obj, 1)
demographic_predictors <- demographic_predictors[demographic_predictors %in% names(first_imp_check)]
personality_predictors <- personality_predictors[personality_predictors %in% names(first_imp_check)]

cat("Predictor sets:\n")
cat("  Demographics:", length(demographic_predictors), "\n")
for(v in demographic_predictors) {
  cat("    •", v, "\n")
}
cat("\n  Personality:", length(personality_predictors), "\n")
for(v in personality_predictors) {
  cat("    •", v, "\n")
}
...
# -----------------------------------------------------------------------------
# Calculate overall discontinuation rate (pooled)
# -----------------------------------------------------------------------------

cat("Calculating overall discontinuation rate...\n")

overall_rates <- numeric(mids_obj$m)
sample_sizes <- numeric(mids_obj$m)
...
for(m in 1:mids_obj$m) {
  ...
  sample_sizes[m] <- nrow(outcome_data)
}

mean_rate <- mean(overall_rates)
se_rate <- sd(overall_rates) / sqrt(mids_obj$m)

cat("Pooled discontinuation rate:\n")
cat("  Mean:", round(100 * mean_rate, 1), "%\n")
cat("  SE:", round(100 * se_rate, 2), "%\n")
cat("  95% CI:", round(100 * (mean_rate - 1.96 * se_rate), 1), "% -",
    round(100 * (mean_rate + 1.96 * se_rate), 1), "%\n")
cat("  Sample size:", round(mean(sample_sizes)), "\n\n")
...
cat("Formula:", deparse(formula_demo), "\n\n")

# Fit on each imputation
cat("Fitting demographic model on", mids_obj$m, "imputations...\n")
...
sig_demo <- summary_demo %>%
  filter(p.value < 0.05, term != "(Intercept)")

cat("Significant predictors (p < 0.05):\n")
if(nrow(sig_demo) > 0) {
  for(i in 1:nrow(sig_demo)) {
    cat("  ✓", sig_demo$term[i], "\n")
    cat("      OR =", round(sig_demo$estimate[i], 3), 
        ", 95% CI: [", round(sig_demo#==============================================================================
# LPA WITH MULTIPLE IMPUTATION - USING ALL 35 IMPUTED DATASETS
# Approach: Fit LPA on each imputation, then combine results
#==============================================================================
...
97.5 %`[i], 3), "]\n")
    cat("      p =", format.pval(sig_demo$p.value[i], digits = 3), "\n")
  }
} else {
  cat("  (None)\n")
}
...
se_auc_demo <- sd(auc_demo_list) / sqrt(mids_obj$m)

cat("Pooled AUC (Demographics Only):\n")
cat("  Mean AUC:", round(mean_auc_demo, 3), "\n")
cat("  SE:", round(se_auc_demo, 4), "\n")
cat("  95% CI:", round(mean_auc_demo - 1.96 * se_auc_demo, 3), "-",
    round(mean_auc_demo + 1.96 * se_auc_demo, 3), "\n\n")
...
formula_pers <- as.formula(paste("scrn_stopped_bzra ~", 
                                 paste(personality_predictors, collapse = " + ")))

cat("Formula:", deparse(formula_pers), "\n\n")

# Fit on each imputation
cat("Fitting personality model on", mids_obj$m, "imputations...\n")
...
# Get significant predictors
sig_pers <- summary_pers %>%
  filter(p.value < 0.05, term != "(Intercept)")

cat("Significant predictors (p < 0.05):\n")
if(nrow(sig_pers) > 0) {
  for(i in 1:nrow(sig_pers)) {
    cat("  ✓", sig_pers$term[i], "\n")
    cat("      OR =", round(sig_pers$estimate[i], 3), 
        ", 95% CI: [", round(sig_pers#==============================================================================
# LPA WITH MULTIPLE IMPUTATION - USING ALL 35 IMPUTED DATASETS
# Approach: Fit LPA on each imputation, then combine results
#==============================================================================
...
97.5 %`[i], 3), "]\n")
    cat("      p =", format.pval(sig_pers$p.value[i], digits = 3), "\n")
  }
} else {
  cat("  (None)\n")
}
...
se_auc_pers <- sd(auc_pers_list) / sqrt(mids_obj$m)

cat("Pooled AUC (Personality Only):\n")
cat("  Mean AUC:", round(mean_auc_pers, 3), "\n")
cat("  SE:", round(se_auc_pers, 4), "\n")
cat("  95% CI:", round(mean_auc_pers - 1.96 * se_auc_pers, 3), "-",
    round(mean_auc_pers + 1.96 * se_auc_pers, 3), "\n\n")

# -----------------------------------------------------------------------------
# MODEL 3: Demographics + Personality
# -----------------------------------------------------------------------------

cat("═══════════════════════════════════════════════════════════════\n")
cat("MODEL 3: DEMOGRAPHICS + PERSONALITY (FULL MODEL)\n")
cat("═══════════════════════════════════════════════════════════════\n\n")
...
# Fit on each imputation
cat("Fitting full model on", mids_obj$m, "imputations...\n")
...
if(nrow(sig_full) > 0) {
  for(i in 1:nrow(sig_full)) {
    cat("  ✓", sig_full$term[i], "\n")
    cat("      OR =", round(sig_full$estimate[i], 3), 
        ", 95% CI: [", round(sig_full#==============================================================================
# LPA WITH MULTIPLE IMPUTATION - USING ALL 35 IMPUTED DATASETS
# Approach: Fit LPA on each imputation, then combine results
#==============================================================================
...
97.5 %`[i], 3), "]\n")
    cat("      p =", format.pval(sig_full$p.value[i], digits = 3), "\n")
  }
} else {
  cat("  (None)\n")
}
...
se_auc_full <- sd(auc_full_list) / sqrt(mids_obj$m)

cat("Pooled AUC (Full Model):\n")
cat("  Mean AUC:", round(mean_auc_full, 3), "\n")
cat("  SE:", round(se_auc_full, 4), "\n")
cat("  95% CI:", round(mean_auc_full - 1.96 * se_auc_full, 3), "-",
    round(mean_auc_full + 1.96 * se_auc_full, 3), "\n\n")
...
comparison_table <- data.frame(
  Model = c("Demographics Only", "Personality Only", "Demographics + Personality"),
  N_Predictors = c(length(demographic_predictors), 
                   length(personality_predictors), 
                   length(all_predictors)),
  Mean_AUC = c(mean_auc_demo, mean_auc_pers, mean_auc_full),
  SE_AUC = c(se_auc_demo, se_auc_pers, se_auc_full),
  CI_Lower = c(mean_auc_demo - 1.96 * se_auc_demo,
               mean_auc_pers - 1.96 * se_auc_pers,
               mean_auc_full - 1.96 * se_auc_full),
  CI_Upper = c(mean_auc_demo + 1.96 * se_auc_demo,
               mean_auc_pers + 1.96 * se_auc_pers,
               mean_auc_full + 1.96 * se_auc_full)
) %>%
  mutate(
...
# Test if full model is significantly better
cat("AUC Improvements:\n")
cat("  Personality vs Demographics: +", 
    round(mean_auc_pers - mean_auc_demo, 3), "\n")
cat("  Full vs Demographics: +", 
    round(mean_auc_full - mean_auc_demo, 3), "\n")
cat("  Full vs Personality: +", 
    round(mean_auc_full - mean_auc_pers, 3), "\n\n")

# Determine best model
best_model <- comparison_table$Model[which.max(comparison_table$Mean_AUC)]
best_auc <- max(comparison_table$Mean_AUC)

cat("BEST MODEL:", best_model, "\n")
cat("  AUC:", best_auc, "\n")
cat("  Discrimination:", ifelse(best_auc >= 0.8, "Excellent",
                          ifelse(best_auc >= 0.7, "Acceptable", "Fair")), "\n\n")

# -----------------------------------------------------------------------------
# Visualization: ROC curves (using first imputation for plot)
# -----------------------------------------------------------------------------

cat("Creating ROC curve visualization...\n")

first_imp <- complete(mids_obj, 1) %>%
  filter(!is.na(scrn_stopped_bzra)) %>%
  mutate(outcome = as.numeric(as.character(scrn_stopped_bzra)))

# Fit models on first imputation
model_demo_1 <- glm(formula_demo, data = first_imp, family = binomial)
model_pers_1 <- glm(formula_pers, data = first_imp, family = binomial)
model_full_1 <- glm(formula_full, data = first_imp, family = binomial)

# Get predictions
pred_demo_1 <- predict(model_demo_1, type = "response")
pred_pers_1 <- predict(model_pers_1, type = "response")
pred_full_1 <- predict(model_full_1, type = "response")

# Create ROC objects
roc_demo_1 <- roc(first_imp$outcome, pred_demo_1, quiet = TRUE)
roc_pers_1 <- roc(first_imp$outcome, pred_pers_1, quiet = TRUE)
roc_full_1 <- roc(first_imp$outcome, pred_full_1, quiet = TRUE)

# Plot
roc_plot <- ggroc(list(
  Demographics = roc_demo_1,
  Personality = roc_pers_1,
  Full = roc_full_1
), size = 1.2) +
  geom_abline(intercept = 1, slope = 1, linetype = "dashed", color = "gray50") +
  scale_color_manual(
    values = c("Demographics" = "#E41A1C", 
               "Personality" = "#377EB8", 
               "Full" = "#4DAF4A"),
    name = "Model",
    labels = c(
      paste0("Demographics (AUC=", round(mean_auc_demo, 3), ")"),
      paste0("Personality (AUC=", round(mean_auc_pers, 3), ")"),
      paste0("Full (AUC=", round(mean_auc_full, 3), ")")
    )
  ) +
  theme_minimal() +
  labs(
    title = "ROC Curves: Predicting BZRA Discontinuation",
    subtitle = paste0("AUCs are pooled estimates across ", mids_obj$m, " imputations"),
...
ggsave("regression_roc_comparison_MI.png", plot = roc_plot, 
       width = 10, height = 8, dpi = 300)

cat("✓ Saved: regression_roc_comparison_MI.png\n\n")

# -----------------------------------------------------------------------------
# Coefficient plot for significant predictors
# -----------------------------------------------------------------------------

cat("Creating coefficient plot for significant predictors...\n")

if(nrow(sig_full) > 0) {
  # Prepare data
  coef_plot_data <- sig_full %>%
    mutate(
      term = factor(term, levels = term[order(estimate)]),
      log_OR = log(estimate),
      log_lower = log(`2.5 %`),
      log_upper = log(`97.5 %`),
      direction = ifelse(log_OR > 0, "Increases Risk", "Decreases Risk")
    )
  
  p_coef <- ggplot(coef_plot_data, aes(x = log_OR, y = term, color = direction)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "gray50") +
    geom_point(size = 3) +
    geom_errorbarh(aes(xmin = log_lower, xmax = log_upper), height = 0.2) +
    scale_color_manual(values = c("Increases Risk" = "#B2182B", 
                                  "Decreases Risk" = "#2166AC")) +
    theme_minimal() +
    labs(
      title = "Significant Predictors of BZRA Discontinuation",
      subtitle = paste0("Pooled across ", mids_obj$m, " imputations (p < 0.05)"),
  ...
  ggsave("regression_coefficients_MI.png", plot = p_coef, 
         width = 10, height = 6, dpi = 300)
  
  cat("✓ Saved: regression_coefficients_MI.png\n\n")
}
...
cat("Saving regression results...\n")

regression_mi_results <- list(
  # Model summaries
  demographics_summary = summary_demo,
  personality_summary = summary_pers,
  full_summary = summary_full,
  
  # Pooled objects
  demographics_pooled = pooled_demo,
  personality_pooled = pooled_pers,
  full_pooled = pooled_full,
  
  # AUC results
  auc_demographics = list(mean = mean_auc_demo, se = se_auc_demo, values = auc_demo_list),
  auc_personality = list(mean = mean_auc_pers, se = se_auc_pers, values = auc_pers_list),
  auc_full = list(mean = mean_auc_full, se = se_auc_full, values = auc_full_list),
  
  # Significant predictors
  sig_demographics = sig_demo,
  sig_personality = sig_pers,
  sig_full = sig_full,
  ...
  # Sample info
  n_imputations = mids_obj$m,
  sample_size = round(mean(sample_sizes)),
  discontinuation_rate = list(mean = mean_rate, se = se_rate)
)

saveRDS(regression_mi_results, "regression_results_MI.rds")
cat("✓ Saved: regression_results_MI.rds\n\n")

# Save comparison table
write.csv(comparison_table, "regression_model_comparison_MI.csv", row.names = FALSE)
cat("✓ Saved: regression_model_comparison_MI.csv\n\n")
...
cat("═══════════════════════════════════════════════════════════════\n")
cat("COMPLETE ANALYSIS SUMMARY: LPA + REGRESSION WITH MI\n")
cat("═══════════════════════════════════════════════════════════════\n\n")

cat("PART 1 - LPA (DESCRIPTIVE):\n")
cat("  Imputations used:", mids_obj$m, "\n")
cat("  Profiles identified:", consensus_k, "\n")
cat("  Mean entropy:", round(mean(entropy_list), 3), "±", round(sd(entropy_list), 3), "\n")
cat("  Assignment stability:", round(100 * mean(assignment_stability), 1), "%\n")
cat("  → Describes patient heterogeneity\n\n")

cat("PART 2 - REGRESSION (PREDICTIVE):\n")
cat("  Sample size:", round(mean(sample_sizes)), "\n")
cat("  Discontinuation rate:", round(100 * mean_rate, 1), "%\n")
cat("  Predictors used:", length(all_predictors), 
    "(", length(demographic_predictors), "demo +", 
    length(personality_predictors), "personality)\n\n")

cat("BEST PREDICTIVE MODEL:", best_model, "\n")
cat("  Pooled AUC:", best_auc, 
    "(", ifelse(best_auc >= 0.8, "excellent", 
         ifelse(best_auc >= 0.7, "acceptable", "fair")), ")\n")
cat("  Significant predictors:", nrow(sig_full), "\n\n")

cat("KEY FINDINGS:\n")
cat("  • Personality variables", 
    ifelse(mean_auc_pers > mean_auc_demo, "outperform", "underperform"),
    "demographics (ΔAUC =", round(mean_auc_pers - mean_auc_demo, 3), ")\n")
cat("  • Full model provides", 
    ifelse(mean_auc_full > max(mean_auc_demo, mean_auc_pers), "best", "comparable"),
    "prediction\n")
cat("  • All estimates properly account for imputation uncertainty\n\n")
...
cat("    ✓ consensus_profiles.rds\n\n")
cat("  Regression:\n")
cat("    ✓ regression_roc_comparison_MI.png\n")
if(exists("p_coef")) {
  cat("    ✓ regression_coefficients_MI.png\n")
}
cat("    ✓ regression_results_MI.rds\n")
cat("    ✓ regression_model_comparison_MI.csv\n\n")

cat("ADVANTAGES OF THIS APPROACH:\n")
cat("  ✓ All", mids_obj$m, "imputations used (no information loss)\n")
cat("  ✓ Properly pooled standard errors (Rubin's rules)\n")
cat("  ✓ Accounts for imputation uncertainty\n")
cat("  ✓ Valid confidence intervals and p-values\n")
cat("  ✓ More generalizable findings\n\n")
