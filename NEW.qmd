...
```{r}
#| label: Complete Random Forest Analysis
...
cat("\n========================================\n")
cat("FINAL VARIABLE IMPORTANCE VISUALIZATION\n")
cat("========================================\n\n")

# Select top 15 variables (excluding alc_use_combined)
top_vars <- pooled_importance_enhanced %>%
  filter(Variable != "alc_use_combined") %>%
  head(15)

# Create a function to rename variables ONLY for the plot
rename_for_plot <- function(var_names) {
  var_mapping <- c(
    "medburden_3" = "Desire to Reduce Medications",
    "traz_use" = "Trazodone Use",
    "dependence_2" = "Worry About Running Out",
    "safety_1" = "Unsafe Driver Next Day",
    "adls_2" = "Negative Impact on Social Life",
    "side_effects_1" = "Morning Drowsiness",
    "med_burden2" = "Comfortable with Med Amount",
    "age" = "Age"
  )
...
# Add display names for plot only
top_vars$Variable_Display <- rename_for_plot(top_vars$Variable)

# Create a clean, publication-ready importance plot
p_importance_final <- ggplot(top_vars, 
                             aes(x = reorder(Variable_Display, MeanDecreaseAccuracy), 
                                 y = MeanDecreaseAccuracy)) +
  geom_col(fill = "steelblue", alpha = 0.8) +
  geom_errorbar(aes(ymin = MeanDecreaseAccuracy - SD_MDA,
                    ymax = MeanDecreaseAccuracy + SD_MDA),
                width = 0.3, alpha = 0.6, linewidth = 0.5) +
  coord_flip() +
  labs(title = "Top 15 Most Important Variables",
       subtitle = "Random Forest Variable Importance (Pooled Across Imputations)",
       x = NULL,
       y = "Mean Decrease in Accuracy",
       caption = "Error bars represent ±1 SD across imputed datasets") +
...
print(p_importance_final)

# Save the plot
ggsave("variable_importance_plot.png", 
       plot = p_importance_final,
       width = 10, height = 7, dpi = 300, bg = "white")

cat("✓ Variable importance plot saved to 'variable_importance_plot.png'\n")

# Print detailed table of top variables (using ORIGINAL variable names)
cat("\nTop 15 Variables (Detailed):\n")
cat(rep("=", 80), "\n", sep = "")

top_vars_table <- top_vars %>%
  mutate(
    MDA = sprintf("%.4f", MeanDecreaseAccuracy),
    SD = sprintf("%.4f", SD_MDA),
    MDG = sprintf("%.4f", MeanDecreaseGini),
    Rank = sprintf("%.1f", Mean_Rank),
    Stability = sprintf("%.3f", CV_Rank)
  ) %>%
  select(Variable, MDA, SD, MDG, Rank, Stability)  # Use original Variable column

print(top_vars_table, row.names = FALSE)

cat("\nColumn Definitions:\n")
cat("  MDA       = Mean Decrease in Accuracy (higher = more important)\n")
cat("  SD        = Standard deviation of MDA across imputations\n")
cat("  MDG       = Mean Decrease in Gini (alternative importance measure)\n")
cat("  Rank      = Average rank across imputations (lower = more consistently important)\n")
cat("  Stability = Coefficient of variation in rank (lower = more stable)\n\n")
```
...
cessation_plot <- ggplot(cessation_viz, aes(x = Cluster, y = prop, fill = as.factor(!!sym(outcome_var)))) +
  ...
  scale_fill_manual(
    name = "Current BZRA Use",
    values = c("0" = "#E41A1C", "1" = "#377EB8"),  # red = still using, blue = not using
    labels = c("0" = "Still Using", "1" = "Not Using")
  ) +
  labs(
    title = "BZRA Cessation by Cluster",
    y = "Percentage",
    x = "Cluster"
  ) +
  theme_minimal()
...
# ============================================================================
# TABLE 1 – PSYCHOLOGICAL CHARACTERISTICS DEFINING CLUSTERS
# ============================================================================
library(dplyr)
library(tidyr)
library(flextable)
...
pooled_data <- pooled_data %>%
  mutate(Cluster = ifelse(Cluster == 1, "Distressed",
                   ifelse(Cluster == 2, "Adaptive", as.character(Cluster))))
...
psych_vars <- c(
  ...
  "DBAS_Worry_Helplessness", "DBAS_Consequences",
  # CISS
  "CISS_Task_Style", "CISS_Emotional_Style", "CISS_Avoidance_Style"
)

# Compute cluster means
psych_summary <- pooled_data %>%
  group_by(Cluster) %>%
  summarise(across(all_of(psych_vars), ~ round(mean(.x, na.rm = TRUE), 2)), .groups = "drop") %>%
  pivot_longer(cols = -Cluster, names_to = "Variable", values_to = "Value") %>%
  pivot_wider(names_from = Cluster, values_from = Value)

# Calculate effect sizes and significance for each variable
effect_sig <- data.frame(
  Variable = character(),
  `Effect.Size..Cohen.s.d.` = character(),
...
for (var in psych_vars) {
  ...
  distressed_vals <- pooled_data %>% filter(Cluster == "Distressed") %>% pull(!!sym(var))
  adaptive_vals <- pooled_data %>% filter(Cluster == "Adaptive") %>% pull(!!sym(var))
  
  # Remove NAs
  distressed_vals <- distressed_vals[!is.na(distressed_vals)]
  adaptive_vals <- adaptive_vals[!is.na(adaptive_vals)]
  
  # Calculate Cohen's d
  mean_diff <- mean(distressed_vals) - mean(adaptive_vals)
  pooled_sd <- sqrt(((length(distressed_vals) - 1) * var(distressed_vals) + 
                     (length(adaptive_vals) - 1) * var(adaptive_vals)) / 
                    (length(distressed_vals) + length(adaptive_vals) - 2))
  cohens_d <- mean_diff / pooled_sd
  
  # t-test for significance
  t_test <- t.test(distressed_vals, adaptive_vals)
  ...
  effect_sig <- rbind(effect_sig, data.frame(
    Variable = var,
    Effect.Size..Cohen.s.d. = as.character(round(cohens_d, 2)),
    Significance = sig,
    stringsAsFactors = FALSE
  ))
}
...
names(effect_sig)[2] <- "Effect Size (Cohen's d)"

# Join effect sizes and significance with summary
psych_summary <- psych_summary %>%
  left_join(effect_sig, by = "Variable")

# Create N row with character values to avoid decimals
N_row <- tibble(
  Variable = "N",
  Distressed = "205",
  Adaptive = "192",
...
# Convert columns to character in psych_summary
psych_summary <- psych_summary %>%
  mutate(Distressed = as.character(Distressed),
         Adaptive = as.character(Adaptive))

# Bind N row on top
psych_summary <- bind_rows(N_row, psych_summary)

# Create flextable with APA-style title
ft1 <- flextable(psych_summary) %>%
  add_header_lines(values = "Personality Characteristics Defining the Distressed and Adaptive Clusters") %>%
  add_header_lines(values = "Table 1") %>%
  bold(i = 1, part = "header") %>%
  italic(i = 2, part = "header") %>%
  align(i = 1:2, align = "left", part = "header") %>%
  border_remove() %>%
  hline_top(border = fp_border(width = 2), part = "header") %>%
  hline(i = 2, border = fp_border(width = 0), part = "header") %>%
  hline_bottom(border = fp_border(width = 1), part = "header") %>%
  hline_bottom(border = fp_border(width = 2), part = "body") %>%
  autofit()

# Export as PNG
save_as_image(ft1, path = "Table1_Psychological_Profiles.png", zoom = 3)

# ============================================================================
# TABLE 2 – DEMOGRAPHIC, CLINICAL, AND CESSATION DIFFERENCES (SPLIT VERSION)
# ============================================================================
...
pooled_data <- pooled_data %>%
  mutate(
    ...
    prov_terr_grouped = case_when(
      prov_terr %in% c("Alberta", "British Columbia", "Manitoba", "Saskatchewan") ~ "Western Canada",
      prov_terr %in% c("Ontario", "Quebec") ~ "Central Canada",
      prov_terr %in% c("New Brunswick", "Newfoundland and Labrador", "Nova Scotia", "Prince Edward Island") ~ "Atlantic Canada",
      TRUE ~ "Territories"
    ),
    education_grouped = case_when(
      education %in% c("Elementary school", "Middle school", "High school diploma") ~ "High School or Less",
      TRUE ~ "Trade School/University"
    ),
    employment_grouped = case_when(
      employment %in% c("Not in workforce", "Retired") ~ "Not in Workforce",
      TRUE ~ "Full- / Part-Time Work"
    ),
    driving_freq = case_when(
      driving_freq == 0 ~ "I do not drive",
      driving_freq == 1 ~ "Less than 1 day per week",
      driving_freq == 2 ~ "1-3 days per week",
      driving_freq == 3 ~ "4-7 days per week",
      TRUE ~ as.character(driving_freq)
    ),
    income = case_when(
      income == 1 ~ "$15,000 or less",
      income == 2 ~ "$15,000 to $30,000",
      income == 3 ~ "$30,000 to $50,000",
      income == 4 ~ "$50,000 to $80,000",
      income == 5 ~ "$80,000 or more",
      TRUE ~ as.character(income)
    ),
    scrn_stopped_bzra = case_when(
      scrn_stopped_bzra == 1 ~ "Not using",
      scrn_stopped_bzra == 0 ~ "Still using",
      TRUE ~ as.character(scrn_stopped_bzra)
    )
  )

# -------------------------------
# Define variables - Combined into one table
# -------------------------------
vars_table2 <- list(
  continuous = c("age", "osss_3_score", "phq2_score"),
  categorical = c("sex", "gender", "prov_terr_grouped", "education_grouped", 
                  "employment_grouped", "driving_freq", "income", "scrn_stopped_bzra")
)

# -------------------------------
# Function to create summary table
# -------------------------------
create_summary_table <- function(continuous_vars = NULL, categorical_vars = NULL) {
  ...
  if (!is.null(continuous_vars)) {
    for (var in continuous_vars) {
      distressed_vals <- pooled_data %>% 
        filter(Cluster == "Distressed") %>% 
        pull(!!sym(var)) %>% 
        na.omit()
      
      adaptive_vals <- pooled_data %>% 
        filter(Cluster == "Adaptive") %>% 
        pull(!!sym(var)) %>% 
        na.omit()
      
      mean_distressed <- round(mean(distressed_vals), 2)
      sd_distressed <- round(sd(distressed_vals), 2)
      mean_adaptive <- round(mean(adaptive_vals), 2)
      sd_adaptive <- round(sd(adaptive_vals), 2)
      ...
      var_label <- ifelse(var == "age", "Age", var)
      
      summary_data <- rbind(summary_data, data.frame(
        Variable = var_label,
        Distressed = paste0(mean_distressed, " (", sd_distressed, ")"),
        Adaptive = paste0(mean_adaptive, " (", sd_adaptive, ")"),
        stringsAsFactors = FALSE
      ))
      
      effect_sig <- rbind(effect_sig, data.frame(
        Variable = var_label,
        `Effect Size` = as.character(round(cohens_d, 2)),
        Significance = sig,
        stringsAsFactors = FALSE,
        check.names = FALSE
      ))
  ...
  if (!is.null(categorical_vars)) {
    for (var in categorical_vars) {
      ...
      var_label <- case_when(
        var == "sex" ~ "Sex",
        var == "gender" ~ "Gender Identity",
        var == "prov_terr_grouped" ~ "Canadian Region",
        var == "education_grouped" ~ "Education",
        var == "employment_grouped" ~ "Employment",
        var == "driving_freq" ~ "Driving Frequency",
        var == "income" ~ "Annual Household Income",
        var == "scrn_stopped_bzra" ~ "Benzodiazepine Use Status",
        TRUE ~ var
      )
      
      # Get distributions
      distressed_dist <- pooled_data %>%
        filter(Cluster == "Distressed", !is.na(!!sym(var))) %>%
        group_by(!!sym(var)) %>%
        summarise(n = n(), .groups = "drop") %>%
        mutate(percent = round(n / sum(n) * 100, 1),
               text = paste0(n, " (", percent, "%)"))
      
      adaptive_dist <- pooled_data %>%
        filter(Cluster == "Adaptive", !is.na(!!sym(var))) %>%
        group_by(!!sym(var)) %>%
        summarise(n = n(), .groups = "drop") %>%
        mutate(percent = round(n / sum(n) * 100, 1),
               text = paste0(n, " (", percent, "%)"))
      ...
      # Chi-square test
      tbl <- table(pooled_data$Cluster, pooled_data[[var]])
      chi_test <- chisq.test(tbl)
      p_value <- chi_test$p.value
      cramers_v <- cramersV(tbl)
      ...
      effect_sig <- rbind(effect_sig, data.frame(
        Variable = var_label,
        `Effect Size` = as.character(round(cramers_v, 2)),
        Significance = sig,
        stringsAsFactors = FALSE,
        check.names = FALSE
      ))
      ...
      for (i in 1:nrow(distressed_dist)) {
        category_label <- distressed_dist[[var]][i]
        distressed_text <- distressed_dist$text[i]
        
        adaptive_match <- adaptive_dist %>% filter(!!sym(var) == category_label)
        adaptive_text <- if(nrow(adaptive_match) > 0) adaptive_match$text[1] else "0 (0.0%)"
        
        summary_data <- rbind(summary_data, data.frame(
          Variable = paste0("  ", category_label),
          Distressed = distressed_text,
          Adaptive = adaptive_text,
          stringsAsFactors = FALSE
        ))
  ...
  # Join effect sizes with summary
...
# -------------------------------
table2 <- create_summary_table(
  continuous_vars = vars_table2$continuous,
  categorical_vars = vars_table2$categorical
)
...
ft2 <- flextable(table2) %>%
  add_header_lines(values = "Demographic and Cessation Differences Between Clusters") %>%
  add_header_lines(values = "Table 2") %>%
  bold(i = 1, part = "header") %>%
  italic(i = 2, part = "header") %>%
  align(i = 1:2, align = "left", part = "header") %>%
  set_header_labels(
    ...
    Adaptive = "Adaptive",
    `Effect Size` = "Effect Size (Cohen's d / Cramér's V)",
...
# Export as PNG
# -------------------------------
save_as_image(ft2, path = "Table2_Demographic_Differences.png", zoom = 3)

# Optional: Print dimensions
cat("Table 2 created with", nrow(table2), "rows\n")
```
...
# ============================================================================
# FIGURE 4 – COMPREHENSIVE SUBGROUP ANALYSIS AND FOREST PLOT FOR CESSATION
# ============================================================================
library(ggplot2)
library(dplyr)
library(tidyr)
...
# Ensure binary cessation variable
pooled_data <- pooled_data %>%
  mutate(
    stopped = as.numeric(scrn_stopped_bzra == "Not using"),
    Cluster_binary = as.numeric(Cluster == "Adaptive")
  )

# ============================================================================
# STEP 2: CALCULATE ALL SUBGROUP EFFECTS
# ============================================================================

# Function to calculate OR with CI for subgroups
calculate_subgroup_OR <- function(data, subgroup_var, subgroup_value = NULL, label) {
  
  # Filter to subgroup if specified
  if (!is.null(subgroup_value)) {
    data_sub <- data %>% filter(!!sym(subgroup_var) == subgroup_value)
  } else {
    data_sub <- data
  }
  ...
  if (nrow(data_sub) < 20) {
    return(data.frame(
      subgroup = label,
      OR = NA, lower_CI = NA, upper_CI = NA, p_value = NA,
      n = nrow(data_sub), significance = "insufficient n",
      stringsAsFactors = FALSE
    ))
  }
  ...
  # Run logistic regression
  tryCatch({
    model <- glm(stopped ~ Cluster_binary, data = data_sub, family = binomial())
    
    # Extract OR and CI
    coef_summary <- summary(model)$coefficients
    OR <- exp(coef(model)[2])
    se_log_or <- coef_summary[2, 2]
    lower_CI <- exp(coef(model)[2] - 1.96 * se_log_or)
    upper_CI <- exp(coef(model)[2] + 1.96 * se_log_or)
    ...
    return(data.frame(
      subgroup = label,
      OR = OR, lower_CI = lower_CI, upper_CI = upper_CI,
      p_value = p_value, n = nrow(data_sub), significance = sig,
      stringsAsFactors = FALSE
    ))
  }, error = function(e) {
    return(data.frame(
      subgroup = label,
      OR = NA, lower_CI = NA, upper_CI = NA, p_value = NA,
      n = nrow(data_sub), significance = "error",
      stringsAsFactors = FALSE
    ))
  })
}
...
# ===== OVERALL =====
all_results[[1]] <- calculate_subgroup_OR(pooled_data, NULL, NULL, "Overall (Unadjusted)")

# ===== DEMOGRAPHICS =====
# Sex
if ("sex" %in% names(pooled_data)) {
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "sex", "Male", "Sex: Male")
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "sex", "Female", "Sex: Female")
}

# Age groups
if ("age" %in% names(pooled_data)) {
  pooled_data <- pooled_data %>%
  ...
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "age_group", "Under 60", "Age: <60")
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "age_group", "60-69", "Age: 60-69")
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "age_group", "70+", "Age: 70+")
}

# Region
if ("prov_terr_grouped" %in% names(pooled_data)) {
  regions <- unique(pooled_data$prov_terr_grouped[!is.na(pooled_data$prov_terr_grouped)])
  for (reg in regions) {
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "prov_terr_grouped", reg, paste0("Region: ", reg))
  }
}

# Education
if ("education_grouped" %in% names(pooled_data)) {
  edu_levels <- unique(pooled_data$education_grouped[!is.na(pooled_data$education_grouped)])
  for (edu in edu_levels) {
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "education_grouped", edu, paste0("Education: ", edu))
  }
}

# Employment
if ("employment_grouped" %in% names(pooled_data)) {
  emp_levels <- unique(pooled_data$employment_grouped[!is.na(pooled_data$employment_grouped)])
  for (emp in emp_levels) {
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "employment_grouped", emp, paste0("Employment: ", emp))
  }
}

# Income
if ("income" %in% names(pooled_data)) {
  pooled_data <- pooled_data %>%
    mutate(income_group = case_when(
      income %in% c("$15,000 or less", "$15,000 to $30,000", "$30,000 to $50,000") ~ "Lower Income",
      income %in% c("$50,000 to $80,000", "$80,000 or more") ~ "Higher Income",
      TRUE ~ as.character(income)
    ))
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "income_group", "Lower Income", "Income: Lower (<$50k)")
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "income_group", "Higher Income", "Income: Higher (≥$50k)")
}

# ===== CLINICAL MEASURES =====
# PHQ-2 (Depression)
if ("phq2_score" %in% names(pooled_data)) {
  pooled_data <- pooled_data %>%
    mutate(depression_level = ifelse(phq2_score < 3, "Low Depression", "High Depression"))
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "depression_level", "Low Depression", "Depression: Low (PHQ-2 <3)")
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "depression_level", "High Depression", "Depression: High (PHQ-2 ≥3)")
}

# OSSS-3 (Social Support)
if ("osss_3_score" %in% names(pooled_data)) {
  pooled_data <- pooled_data %>%
    mutate(social_support = ifelse(osss_3_score >= 9, "High Support", "Low Support"))
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "social_support", "High Support", "Social Support: High (OSSS ≥9)")
  all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, "social_support", "Low Support", "Social Support: Low (OSSS <9)")
}

# ===== MEDICATION BURDEN =====
med_burden_vars <- c("med_burden_1", "med_burden2", "medburden_3", "med_burden_4")
for (var in med_burden_vars) {
  if (var %in% names(pooled_data) && !all(is.na(pooled_data[[var]]))) {
    median_val <- median(pooled_data[[var]], na.rm = TRUE)
    pooled_data[[paste0(var, "_group")]] <- ifelse(pooled_data[[var]] <= median_val, "Low", "High")
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "Low", paste0("Med Burden (", var, "): Low"))
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "High", paste0("Med Burden (", var, "): High"))
  }
}

# ===== SUBSTANCE USE =====
substance_vars <- c("op_use", "can_use", "caf_use", "nico_use", "alc_sleep", "can_sleep", 
                    "melatonin_use", "op_sleep", "quet_use", "traz_use", "otc_use")
for (var in substance_vars) {
  if (var %in% names(pooled_data) && length(unique(pooled_data[[var]][!is.na(pooled_data[[var]])])) > 1) {
    # Assume binary or convert to binary
    unique_vals <- sort(unique(pooled_data[[var]][!is.na(pooled_data[[var]])]))
    if (length(unique_vals) == 2) {
      all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, var, unique_vals[1], paste0(var, ": ", unique_vals[1]))
      all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, var, unique_vals[2], paste0(var, ": ", unique_vals[2]))
    }
  }
}

# ===== PERSONALITY (BFI-10) =====
bfi_vars <- c("Extraversion", "Agreeableness", "Conscientiousness", "Neuroticism", "Openness")
for (var in bfi_vars) {
  if (var %in% names(pooled_data)) {
    median_val <- median(pooled_data[[var]], na.rm = TRUE)
    pooled_data[[paste0(var, "_group")]] <- ifelse(pooled_data[[var]] <= median_val, "Low", "High")
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "Low", paste0(var, ": Low"))
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "High", paste0(var, ": High"))
  }
}

# ===== SURPS =====
surps_vars <- c("SURPS_Impulsivity", "SURPS_Sensation_Seeking", "SURPS_Hopelessness", "SURPS_Anxiety_Sensitivity")
for (var in surps_vars) {
  if (var %in% names(pooled_data)) {
    median_val <- median(pooled_data[[var]], na.rm = TRUE)
    pooled_data[[paste0(var, "_group")]] <- ifelse(pooled_data[[var]] <= median_val, "Low", "High")
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "Low", paste0(var, ": Low"))
...
# ===== DBAS =====
dbas_vars <- c("DBAS_Expectations", "DBAS_Medications", "DBAS_Worry_Helplessness", "DBAS_Consequences")
for (var in dbas_vars) {
  if (var %in% names(pooled_data)) {
    median_val <- median(pooled_data[[var]], na.rm = TRUE)
    pooled_data[[paste0(var, "_group")]] <- ifelse(pooled_data[[var]] <= median_val, "Low", "High")
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "Low", paste0(var, ": Low"))
...
# ===== CISS =====
ciss_vars <- c("CISS_Task_Style", "CISS_Emotional_Style", "CISS_Avoidance_Style")
for (var in ciss_vars) {
  if (var %in% names(pooled_data)) {
    median_val <- median(pooled_data[[var]], na.rm = TRUE)
    pooled_data[[paste0(var, "_group")]] <- ifelse(pooled_data[[var]] <= median_val, "Low", "High")
    all_results[[length(all_results) + 1]] <- calculate_subgroup_OR(pooled_data, paste0(var, "_group"), "Low", paste0(var, ": Low"))
...
# Combine all results
all_subgroups <- bind_rows(all_results)

# Print all results
cat("\n=== ALL SUBGROUP ANALYSES ===\n")
all_subgroups_sorted <- all_subgroups %>%
  filter(!is.na(OR)) %>%
  arrange(p_value) %>%
  select(subgroup, n, OR, lower_CI, upper_CI, p_value, significance) %>%
  mutate(across(c(OR, lower_CI, upper_CI), ~round(., 2)),
         p_value = round(p_value, 4))

print(all_subgroups_sorted, n = 100)

# Save to CSV
write.csv(all_subgroups_sorted, "All_Subgroup_Analysis_Results.csv", row.names = FALSE)

# ============================================================================
# STEP 3: SELECT MOST SIGNIFICANT SUBGROUPS FOR FOREST PLOT
# ============================================================================

# Filter to significant effects (p < 0.05) with highest ORs
significant_subgroups <- all_subgroups %>%
  filter(p_value < 0.05 & !is.na(OR) & !grepl("Overall", subgroup)) %>%
  arrange(desc(OR)) %>%
  slice_head(n = 8)  # Top 8 most significant

# Add overall at top
forest_data <- bind_rows(
  all_subgroups %>% filter(grepl("Overall", subgroup)),
  significant_subgroups
) %>%
  mutate(
    category = ifelse(grepl("Overall", subgroup), "Overall", "Subgroup"),
    order = rev(seq_len(n()))
  )

cat("\n=== SELECTED SUBGROUPS FOR FOREST PLOT ===\n")
print(forest_data %>% select(subgroup, n, OR, lower_CI, upper_CI, p_value, significance))
...
x_min <- 0.5
x_max <- max(forest_data$upper_CI, na.rm = TRUE) * 1.15

p <- ggplot(forest_data, aes(y = order, x = OR)) +
  ...
  geom_point(aes(shape = category, fill = category), 
             size = 4, stroke = 0.8, color = "black") +
  
  # Customize shapes
  scale_shape_manual(values = c("Overall" = 23, "Subgroup" = 22)) +
  scale_fill_manual(values = c("Overall" = "black", "Subgroup" = "white")) +
  
  # Scales
  scale_x_continuous(
    breaks = seq(0, ceiling(x_max), 0.5),
    limits = c(x_min, x_max),
    name = "Odds Ratio (95% CI)"
  ) +
  
  scale_y_continuous(
    breaks = forest_data$order,
    labels = paste0(forest_data$subgroup, "  ",
                   sprintf("%.2f", forest_data$OR), " (",
                   sprintf("%.2f", forest_data$lower_CI), "-",
                   sprintf("%.2f", forest_data$upper_CI), ")",
                   forest_data$significance),
    name = ""
  ) +
  ...
  labs(
    title = "Figure 1: Subgroup Effects on Benzodiazepine Cessation",
    subtitle = "Adaptive Cluster vs. Distressed Cluster - Strongest Effects"
  )

# Save plot
ggsave("Figure4_Subgroup_Forest_Plot.png", 
       plot = p, 
       width = 12, 
       height = 7, 
       dpi = 300,
...
cat("\n=== ANALYSIS COMPLETE ===\n")
cat("Complete results saved to: All_Subgroup_Analysis_Results.csv\n")
cat("Forest plot saved to: Figure4_Subgroup_Forest_Plot.png\n")
```
