...
if(is.list(pilot) && !is.null(pilot$error)) {
  ...
  # Save convergence plot as PDF in Testing repo
  tryCatch({
    ...
    if(file.exists(output_path)) {
      cat("✓ Convergence plot saved:", normalizePath(output_path), "\n")
...
tryCatch({
  # Save directly in current working directory (which is already Testing)
  output_path <- "pilot_convergence_plot_use.png"
  
  cat("\nAttempting to create PNG file...\n")
  ...
  # Close device
  dev_result <- dev.off()
  cat("Device closed, result:", dev_result, "\n")
  
  # Small delay to ensure file system writes
  Sys.sleep(0.5)
  ...
  if(file.exists(output_path)) {
    ...
    print(list.files(pattern = "pilot.*png"))
  }
}, error = function(e) {
